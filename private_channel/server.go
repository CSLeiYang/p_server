...
func StartUDPServer() {
	...
	for {
		...
		go func(recvData []byte, oneClientAddr *net.UDPAddr) {
			...
			oneUdpClient := updateUDPClient(pp.StreamId, conn, oneClientAddr)

			if pp.Tid == HeartbeatTid {

				log.Infof("pp:%v is hearbeat,so ignore.", pp.StreamId)
			} else {
...
func updateUDPClient(sid uint64, udpCon *net.UDPConn, clientAddr *net.UDPAddr) *udpClient {
	...
	if client, exits := udpClients[clientKey]; exits {
		...
		udpClients[clientKey] = &udpClient{sid: clientKey, conn: udpCon, remoteAddr: clientAddr, lastHeartBeat: time.Now(), pudpConn: NewPudpConn(sid, udpCon, clientAddr, nil, HandlePEvent)}
